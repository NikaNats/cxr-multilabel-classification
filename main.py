...
def main():
    ...
    # 1. Forensic Dataset Audit (Memory-Safe)
    print("\n[*] Loading RAW images for Forensic Audit...")
    train_raw, val_raw, test_raw, _ = get_raw_datasets()
    
    # Audit logic handles sampling internally to prevent "Killed" (OOM)
    # execute_forensic_audit(train_raw, val_raw, test_raw)
    
    print("[*] Audit complete. Clearing raw images from RAM...")
    del train_raw, val_raw, test_raw
    gc.collect()

    # 2. Data Preparation (Embeddings for Training)
    ...
    # 3. Dynamic Graph Adjacency (NATURE-GRADE FIX 1)
    # Derive optimal threshold using max-curvature (Elbow Method)
    ...
    # Build symmetric adjacency before normalization
    ...
    # 4. Training Pipeline
    ENSEMBLE_SEEDS = [42] # M=1 baseline
    ensemble_checkpoints = train_ensemble(
        ENSEMBLE_SEEDS, adj_norm, train_emb_dataset, 
        train_loader, val_loader, num_workers
    )
    
    # 5. Load Final Model for Inference
    pro_model = CXR_Synapse_Foundation(num_classes=14).to(DEVICE)
    pro_model.set_adjacency_mask(adj_norm)
    
    # Generate SOTA BioViL-T text embeddings
    radlex = ensure_radlex_embeddings(
        path="radlex_embeddings_14.pth", 
        pathologies=RADLEX_PATHOLOGIES, 
        model_name="microsoft/BiomedVLP-BioViL-T", 
        device_=DEVICE
    )
    pro_model.set_radlex_embeddings(radlex)
    
    # Load state dict (clean module. prefixes)
    ...
    # [B] Post-hoc Temperature Scaling (Calibration)
    ...
    # [C] Binary Classification (Disease vs Normal)
    ...
    # [D] Evidential Deep Learning (EDL) Evaluator
    ...
    # [E] TorchCP Conformal Prediction (FIXED)
    ...
    def _collect_logits_and_labels(loader, model, device_):
        ...
        with torch.no_grad():
            for xb, yb in loader:
                xb = xb.to(device_)
                logits = model(xb)
                all_logits.append(logits.detach().cpu()) # Move to CPU here to avoid device issues
    ...
    # TorchCP expects long indices for classes
    ...
    test_sets_raw = torchcp_predictor.predict_with_logits(test_logits_t.to(DEVICE))
    
    # ── THE FIX: CPU Transfer Loop ──
    test_pred_sets = np.zeros(test_labels_t.shape, dtype=bool)
    for i, idxs in enumerate(test_sets_raw):
        if torch.is_tensor(idxs):
            idxs = idxs.cpu().numpy() # Transfer from GPU to CPU
    ...
    checklist = [
        ...
        "GLoRI-Lite pooler: Correct GeM (Nature Fix 2)",
        "Adjacency: Symmetric joint co-occurrence (Nature Fix 1)",
        "Uncertainty: Evidential Deep Learning (EDL)",
        "Statistical: Paired Bootstrap 95% CI + p-values",
        "Conformal: TorchCP Class-Conditional (α=0.10)"
    ]
