...
def plot_conformal_tradeoff(val_probs: np.ndarray, val_labels: np.ndarray, opt_thresholds: np.ndarray,
                            experiment_id: str, output_dir: str | Path = "."):
...
def plot_semantic_manifold(embeddings: np.ndarray, labels: np.ndarray, class_names: list[str], experiment_id: str,
                           output_dir: str | Path = ".", max_samples: int = 3000):
...
def plot_paq_attention(image_array, attn_weights, pathology_name: str, experiment_id: str,
                       output_dir: str | Path = "."):
    ...
    attn = (attn_weights.cpu().numpy() if hasattr(attn_weights, "cpu") else np.asarray(attn_weights)).reshape(8,
                                                                                                              8).astype(
        float)
